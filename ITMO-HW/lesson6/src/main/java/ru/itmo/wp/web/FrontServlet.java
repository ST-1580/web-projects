...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        if (!invokeMethod(pageClass, before, view, page, request, response)) {
            return;
        }
        if (!invokeMethod(pageClass, method, view, page, request, response)) {
            return;
        }
        if (!invokeMethod(pageClass, after, view, page, request, response)) {
            return;
        }
    ...
    private Method findMethod(Class<?> pageClass, String currAction) throws NotFoundException {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                method = clazz.getDeclaredMethod(currAction, HttpServletRequest.class, Map.class);
    ...
    private boolean invokeMethod(Class<?> pageClass, Method method, Map<String, Object> view, Object page,
        ...
        try {
            ...
            if (cause instanceof RedirectException) {
                ...
                for (Map.Entry<String, String[]> param : request.getParameterMap().entrySet()) { String key = param.getKey();
