...
public class UserRepositoryImpl implements UserRepository {
    ...
    public Long findCount() {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Can't calculate number of users.", e);
        }
    }
    ...
    public User findByLoginOrEmail(String loginOrEmail) {
        String param = getParam(loginOrEmail);
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User WHERE " + param + "=?")) {
    ...
    public User findByLoginOrEmailAndPasswordSha(String loginOrEmail, String passwordSha) {
        String param = getParam(loginOrEmail);
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User WHERE " + param + "=? AND passwordSha=?")) {
                statement.setString(1, loginOrEmail);
    ...
    private String getParam(String loginOrEmail) {
