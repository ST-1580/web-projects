...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public void save(Article article) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `Article` (`userId`, `title`, `text`, `isHide`, `creationTime`) VALUES (?, ?, ?, ?, NOW())", Statement.RETURN_GENERATED_KEYS)) {
                statement.setLong(1, article.getUserId());
                statement.setString(2, article.getTitle());
                statement.setString(3, article.getText());
                statement.setBoolean(4, article.isHide());
    ...
    public List<Article> findAll() {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `Article` WHERE isHide=0 ORDER BY id DESC")) {
    ...
    public List<Article> findMyArticle(User user) {
        List<Article> articles = new ArrayList<>();
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `Article` WHERE userId=" + user.getId())) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    Article article;
                    while ((article = toArticle(statement.getMetaData(), resultSet)) != null) {
                        articles.add(article);
    ...
    public void reload(long id) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE `Article` SET `isHide` = NOT isHide WHERE `Article`.`id` =" + id, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public Article findById(long id) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM Article WHERE id=" + id)) {
                try (ResultSet resultSet = statement.executeQuery()) {
    ...
    public User findOwner(long id) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM Article WHERE id=" + id)) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    Article article = toArticle(statement.getMetaData(), resultSet);
                    assert article != null;
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            switch (metaData.getColumnName(i)) {
                ...
                case "creationTime":
                    ...
                    break;
                case "isHide":
                    article.setHide(resultSet.getBoolean(i));
                    break;
                default:
                    // No operations.
            }
